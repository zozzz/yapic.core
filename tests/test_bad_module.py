...
def test_bad_module():
    with pytest.raises(SystemError) as ex:
        from yapic.core.test import _bad_module


def test_bad_module2():
    with pytest.raises(Exception) as ex:
        from yapic.core.test import _bad_module2

    assert ex.type.__module__ == "yapic.core.test._bad_module2"
...
def test_bad_import():
    with pytest.raises(ModuleNotFoundError) as ex:
        from yapic.core.test import _bad_import
...
def test_bad_import2():
    with pytest.raises(AttributeError) as ex:
        from yapic.core.test import _bad_import2
